...
DispatchWriter::DispatchWriter(std::fstream &fs, _type_dispatch_fileoffset offset_cursor, const kiwi::RID rankId)
        : ofs(fs), base_offset(offset_cursor), rankId(rankId) {
}

_type_dispatch_fileoffset DispatchWriter::stat(const kiwi::RID rank_count) {
    // todo write rank count.
    return sizeof(kiwi::RID) + sizeof(_type_dispatch_fileoffset) * rank_count;
}

void DispatchWriter::locate(_type_nodes_count nodes_count) {
    offset_in_rank = sizeof(_type_nodes_count) + nodes_count * sizeof(_type_dispatch_fileoffset);
    kiwi::seekWrite(ofs, &nodes_count, base_offset, std::ios_base::beg, 1); // write node count.
}

_type_dispatch_fileoffset DispatchWriter::postWrite(_type_dispatch_fileoffset *cursor) {
    kiwi::seekWrite(ofs, &base_offset,
                    sizeof(kiwi::RID) + rankId * sizeof(_type_dispatch_fileoffset),
                    std::ios_base::beg, 1);
...
void DispatchWriter::write(const DNode *np) {
    ...
    _type_nodes_count up_count = np->getUpstreamNodesCount();
    _type_nodes_count down_count = np->getDownstreamNodesCount();
    ...
    kiwi::seekWrite(ofs, &offset_in_rank,
                    base_offset + sizeof(_type_nodes_count) + node_i * sizeof(_type_dispatch_fileoffset),
                    std::ios_base::beg, 1);
    node_i++;
