...
void StreamRoutingMessageRunner::onMessage(MPI_Status *pStatus) {
    ...
    // receive using MPI_Recv
    up_routing.routingRecv<false>(pStatus, nullptr, domain::mpi_sim_process.comm);
    queueAppend(up_routing);
}

void StreamRoutingMessageRunner::onMessage(MPI_Status *pStatus, MPI_Message *pMessage) {
    TypeRouting up_routing;
    // receive using MPI_Mrecv
    up_routing.routingRecv<true>(pStatus, pMessage, domain::mpi_sim_process.comm);
    queueAppend(up_routing);
}

void StreamRoutingMessageRunner::queueAppend(TypeRouting &up_routing) {
    ...
    // write queue.
    pNode->upstream.appendUpstreamRouting(up_routing.source_id, up_routing); // todo may large data copy
