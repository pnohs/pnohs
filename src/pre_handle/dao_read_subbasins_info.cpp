...
void dao_read_subbasins_info::init(const std::string &filename, const PROCESSOR_ID_TYPE self_processor_id) {
    //打开二进制输入文件
    this->fs.open(filename, OPEN_READ_BINARY_FILE_MODE);

    if (!fs) {
        //出错处理需要额外的模板，暂时先这样
        err_handle::sys_err("file" + filename + "not exit", -1);
    }
    //读取本进程所需子流域数据在输入文件中的最开始偏移量this->base_offset
...
void dao_read_subbasins_info::read_subbasin_forceinfo_byid(const SUBBASIN_ID_TYPE subb_id,
    ...
    OFFSET_TYPE subb_offset = this->get_offset_by_id(subb_id)
                              + this->solve_base_data_size(out_subb_info)
                              + sizeof(*out_subb_info.geo_data) + sizeof(*out_subb_info.soil_data);
...
void dao_read_subbasins_info::read_subbasins_allinfo_all(SUBBASINS_CONTAINER_TYPE &out_subbs_info) {
    ...
    this->fs.seekg(this->first_subbasin_offset);

    // 2.读数据
    for (int i = 0; i < this->subbasins_count; i++) {
        subb_info = new SUBBASIN_INFO_TYPE;
        this->read_one_subbasin_all_data(*subb_info);
        //out_subbs_info[subb_info->subbasin_id] = subb_info; // 存在效率问题，因为是第一次插入，使用改种方式会先检查容器中是否存在该子流域ID
        out_subbs_info.insert(SUBBASINS_CONTAINER_TYPE::value_type(subb_info->subbasin_id, subb_info));
    }
}
...
void dao_read_subbasins_info::read_subbasins_baseinfo_all(SUBBASINS_CONTAINER_TYPE &out_subbs_info) {
    ...
    for (int i = 0; i < this->subbasins_count; i++) {
        ...
        this->fs.seekg(this->first_subbasin_offset + this->subbasins_info_offsets[i]);
...
void dao_read_subbasins_info::read_subbasins_soilinfo_all(SUBBASINS_CONTAINER_TYPE &out_subbs_info) {
    ...
    for (; it != out_subbs_info.end(); it++) {
        ...
        subb_offset = this->get_offset_by_id(it->first);
        // 2.计算单个子流域其他类型数据在输入文件中的绝对偏移量
        subb_offset += this->solve_base_data_size(*it->second) + sizeof(*it->second->geo_data);
        this->fs.seekg(subb_offset);
        // 3.开始读取数据
...
void dao_read_subbasins_info::read_subbasins_forceinfo_all(SUBBASINS_CONTAINER_TYPE &out_subbs_info) {
    ...
    for (; it != out_subbs_info.end(); it++) {
        ...
        // 2.计算单个子流域其他类型数据在输入文件中的绝对偏移量
        subb_offset += this->solve_base_data_size(*it->second)
                       + sizeof(*it->second->geo_data)
                       + sizeof(*it->second->soil_data);
...
void dao_read_subbasins_info::read_one_subbasin_base_data(SUBBASIN_INFO_TYPE &subb_info) {
    ...
    this->fs.read(reinterpret_cast<char *>(&tmp_id), sizeof(SUBBASIN_ID_TYPE));
    subb_info.setSubbasin_id(tmp_id);
    this->fs.read(reinterpret_cast<char *>(&tmp_count), sizeof(SUBBASIN_COUNT_TYPE));
    subb_info.down_subbasin_count = tmp_count;
    this->fs.read(reinterpret_cast<char *>(&tmp_count), sizeof(SUBBASIN_COUNT_TYPE));
    subb_info.setUp_subbasin_count(tmp_count);


    if (!subb_info.down_subbasin_ids) {
        subb_info.setDown_subbasin_ids(new SUBBASIN_ID_CONTAINER_TYPE(subb_info.down_subbasin_count));
    } else {
        subb_info.down_subbasin_ids->resize(subb_info.down_subbasin_count);
    }
    this->fs.read(reinterpret_cast<char *>(subb_info.down_subbasin_ids->data()),
                    sizeof(SUBBASIN_ID_TYPE) * subb_info.down_subbasin_count);
    if (!subb_info.down_subbasin_belong_processor) {
        subb_info.setDown_subbasin_belong_processor(
                new PROCESSOR_ID_CONTAINER_TYPE(subb_info.down_subbasin_count));
    } else {
        subb_info.down_subbasin_belong_processor->resize(subb_info.down_subbasin_count);
    }
    this->fs.read(reinterpret_cast<char *>(subb_info.down_subbasin_belong_processor->data()),
                    sizeof(PROCESSOR_ID_TYPE) * subb_info.down_subbasin_count);


    if (!subb_info.up_subbasin_ids) {
        subb_info.setUp_subbasin_ids(new SUBBASIN_ID_CONTAINER_TYPE(subb_info.up_subbasin_count));
    } else {
        subb_info.up_subbasin_ids->resize(subb_info.up_subbasin_count);
    }
    this->fs.read(reinterpret_cast<char *>(subb_info.up_subbasin_ids->data()),
                    sizeof(SUBBASIN_ID_TYPE) * subb_info.up_subbasin_count);
    if (!subb_info.up_subbasin_belong_processor) {
        subb_info.setUp_subbasin_belong_processor(new PROCESSOR_ID_CONTAINER_TYPE(subb_info.up_subbasin_count));
    } else {
        subb_info.up_subbasin_belong_processor->resize(subb_info.up_subbasin_count);
    }
    this->fs.read(reinterpret_cast<char *>(subb_info.up_subbasin_belong_processor->data()),
                    sizeof(PROCESSOR_ID_TYPE) * subb_info.up_subbasin_count);

}

void dao_read_subbasins_info::read_one_subbasin_soil_data(SUBBASIN_INFO_TYPE &subb_info) {
    if (!subb_info.soil_data)
        subb_info.setSoil_data(new soil_info_subbasin);
    this->fs.read(reinterpret_cast<char *>(subb_info.soil_data), sizeof(*subb_info.soil_data));
}

void dao_read_subbasins_info::read_one_subbasin_force_data(SUBBASIN_INFO_TYPE &subb_info) {
    if (!subb_info.force_data)
        subb_info.setForce_data(new force_info_subbasin);
    this->fs.read(reinterpret_cast<char *>(subb_info.force_data), sizeof(*subb_info.force_data));
}

void dao_read_subbasins_info::read_one_subbasin_geo_data(SUBBASIN_INFO_TYPE &subb_info) {
    if (!subb_info.geo_data) {
        subb_info.setGeo_data(new geo_info_subbasin);
    }
    this->fs.read(reinterpret_cast<char *>(subb_info.geo_data), sizeof(*subb_info.geo_data));
}
...
OFFSET_TYPE dao_read_subbasins_info::solve_base_data_size(SUBBASIN_INFO_TYPE &subb_info) {
    return sizeof(SUBBASIN_ID_TYPE) + sizeof(SUBBASIN_COUNT_TYPE) * 2
           + (subb_info.down_subbasin_count + subb_info.up_subbasin_count)
